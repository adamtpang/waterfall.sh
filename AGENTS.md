...
- **Leaderboard coverage, BUILT 2026-09-03** (`router/leaderboard_coverage.py`,
  ...
  162 borrowed rows from 424 catalog models (58 variants folded into their
  base row as `variants`: 49 `:batch` and 9 `:free`; Adam chose to fold free
  too, so the base row carries `has_free_variant` and the feed reports
  `free_tier_count`, 11 at publish; a free-only model with no paid base still
  stands alone): AA `coding_index` first, then
...
Grok chat continuation: read `CLAUDE_CONTINUE_FROM_GROK.md` to resume from the latest local Grok sessions for this project.
<!-- END:grok-chat-continuation -->
<!-- BEGIN:waterfall-desktop-handoff-2026-08-11 -->
**Desktop session handoff (2026-08-11):** read **`HANDOFF_CLI_2026-08-11.md` first**, then `GAP_VS_CLAUDE_CODE.md`. In-app Grok local, verbose stream, project filter/sort/drag-reorder. Next: P1 sessions + transcripts + multi-run.
<!-- END:waterfall-desktop-handoff-2026-08-11 -->
...
Before making changes, read `CLAUDE.md` in this project if it exists. It is
the live handoff from Claude Code and the source of truth for current
project progress, design decisions, constraints, and open tasks. Keep
future progress updates there so Claude and Codex stay in sync.
